...
const endHtmlStr = '</html>\n    `;\n    printWindow.document.write(htmlContent);\n    printWindow.document.close();\n  };';
let endIdx = content.indexOf(endHtmlStr, startIdx);

if (endIdx === -1) {
  const endHtmlStr2 = '</html>\n    `;\n    printWindow.document.write(htmlContent);\n  };';
  endIdx = content.indexOf(endHtmlStr2, startIdx);
  if (endIdx !== -1) endIdx += endHtmlStr2.length;
} else {
  endIdx += endHtmlStr.length;
}
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log('Successfully replaced handlePrintPDFReport');
} else {
  console.error('Could not find handlePrintPDFReport end boundaries', { startIdx, endIdx });
}
